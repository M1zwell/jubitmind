...
/**
 * Extractor for Qwen (chat.qwen.ai / tongyi.aliyun.com).
 * Uses generic selectors as the DOM structure may vary.
 */
export const qwenExtractor: DOMExtractor = {
  tool: 'qwen',
  ...
  async extractConversationMessages(evaluate: EvalFn): Promise<ExtractedMessage[]> {
    const result = await evaluate(`
      (() => {
        try {
          const messages = [];
          // Generic chat message extraction for Qwen / Tongyi
      ...
      (() => {
        try {
          ...
          const hasCookies = document.cookie.includes('token')
            ...
            || document.cookie.includes('login_tongyi');
